	...
	for _, arg := range args {
		for _, part := range strings.Split(arg, ",") {
			...
			if !shared.IsAllowedADBUserRole(role) {
				return nil, fmt.Errorf("invalid role %q (allowed: %s)", role, strings.Join(sortedAllowedDevUserRoles(), ", "))
			}
...
func sortedAllowedDevUserRoles() []string {
	roles := append([]string(nil), shared.AllowedADBUserRoles...)
	sort.Strings(roles)
	return roles
}
