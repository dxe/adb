...
type OptionalFilterKey = (typeof OPTIONAL_FILTERS)[number]['key']

/** Helper to remove a key from a Set (returns new Set). */
function without(
  set: Set<OptionalFilterKey>,
  key: OptionalFilterKey,
): Set<OptionalFilterKey> {
...
export function ActivistFilters({
  ...
  if (filters !== prevFilters) {
    ...
    if (pruned) {
      const next = new Set<OptionalFilterKey>()
      for (const key of visibleFilters) {
        if (filters[key] !== undefined) {
          next.add(key)
        }
  ...
  const showFilter = useCallback((key: OptionalFilterKey) => {
    // Close the popover first, then set filters on the next frame. The popover
    // has a CSS exit animation, so it stays mounted while it fades out. If the
    // filter chips shift layout in the same render, the popover follows its
    // trigger, causing a flicker.
  ...
  const optionalOnChange = useCallback(
    (key: OptionalFilterKey, value: FilterState[OptionalFilterKey]) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Name search */}
      <NameSearchFilter filters={filters} onFiltersChange={onFiltersChange} />
                ...
                {isAdmin && !filters.searchAcrossChapters && (
                  <button
                    ...
                    onClick={() => {
                      ...
                      // frame. The popover has a CSS exit animation, so it
                      // stays mounted while it fades out. If the filter chips
                      // shift layout in the same render, the popover follows
                      // its trigger, causing a flicker.
                      setIsAddFilterOpen(false)
                      requestAnimationFrame(() => {
                        onFiltersChange({
                          ...filters,
...
export type { FilterState } from '../query-state'
