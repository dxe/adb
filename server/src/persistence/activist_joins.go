...
const (
	...
	// Action event types (DA events) used for last_action and mpp_requirements columns.
	actionEventTypes = "'action', 'outreach', 'frontline surveillance', 'animal care', 'campaign action'"
...
LEFT JOIN (
	...
	GROUP BY ea.activist_id
) %s ON %s.activist_id = %s.id`, actionEventTypes, lastActionSubqueryKey, lastActionSubqueryKey, activistTableAlias),
	}
...
LEFT JOIN (
	...
	GROUP BY ea.activist_id
) %s ON %s.activist_id = %s.id`, actionEventTypes, communityEventTypes, mppRequirementsSubqueryKey, mppRequirementsSubqueryKey, activistTableAlias),
	}
)
